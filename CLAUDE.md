...
- **The hero is a 3-slide cross-fade** (`.hero-slide`, 4s interval in JS, paused while the tab is hidden and disabled under `prefers-reduced-motion`). Slides 2 and 3 are portrait photos, so each carries its own `object-position` to aim the wide crop at the barrier gates rather than the middle of the frame. Both gradients still paint over the slides without extra layering: `::before` has `z-index: 1`, and `::after` follows the images in DOM order.
- **The gallery is a grid on desktop and a snap-scrolling row on phones.** The mobile rule bleeds it to the screen edges with a negative inline margin that exactly cancels `.container`'s gutter, so it never widens the page; `#galCurrent` is updated from the scroll position to show "n / 12".
